...
namespace PSULib.FileClasses.Archives
{
    public class AfsLoader : PsuFile, ContainerFile
    {
        ...
        private void populateFile(AfsFileEntry afsFileEntry)
        {
            ...
            {
                string headerName = Encoding.GetEncoding("ASCII").GetString(afsFileEntry.rawContents, 0, 4);
        ...
        // AFS files do not guarantee filename uniqueness. There's no way this function can be implemented that would not surprise the caller.
        // DO NOT EVER CALL THIS.
        public void removeFile(string filename)
        {
            afsList.RemoveAll(afsFile => afsFile.fileName == filename);
        }
        ...
        {
            afsList.FindAll(afsFile => afsFile.fileName == filename).ForEach(afsFile => afsFile.fileName = newFilename);
        }

        public void renameFile(int index, string newFilename)
        {
            if (index < afsList.Count)
            {
                afsList[index].fileName = newFilename;
            }
        }
    }
}
