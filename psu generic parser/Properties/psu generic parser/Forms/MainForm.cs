...
namespace psu_generic_parser
{
    public partial class MainForm : Form
    {
        ...
        // ====================== Display Hex Editor in right panel for ADX ======================
        private void LoadAdxIntoRightPanel(string filePath, TreeNode node)
        {
            try
            {
                ...
                string headingText = $"ADX Audio - {filename}";

                // Close previous hex form
                if (currentFileHexForm != null && !currentFileHexForm.IsDisposed)
                    currentFileHexForm.Close();

                // Create and embed the Hex Editor
                currentFileHexForm = new HexEditForm(data, headingText, true, null);
                currentFileHexForm.TopLevel = false;
                currentFileHexForm.FormBorderStyle = FormBorderStyle.None;
                currentFileHexForm.Dock = DockStyle.Fill;
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            // Special handling for single ADX file (unchanged)
            if (tag.OwnerContainer == null && tag.FileName.EndsWith(".adx", StringComparison.OrdinalIgnoreCase))
            {
            ...
            // === HANDLE FILES INSIDE CONTAINERS ===
            if (tag.OwnerContainer != null)
            {
                ...
                // === Skip large-file warning for known small container/chunk types ===
                ...
                {
                    // These are containers or small internal chunks — show parsed preview
                    setRightPanel(parent.getFileParsed(index));
                    return;
                }

                // === LARGE FILE PROTECTION (only for other file types) ===
                const long MAX_SAFE_SIZE = 500 * 1024 * 1024; // 500 MB

                long fileSize = 0;
                string displayName = fileName;   // for the warning message
                ...
                // Only small regular files reach here
                setRightPanel(parent.getFileParsed(index));
            }
        }
        ...
        private void exportSelected()
        {
            ...
            {
                // === NEW: Prefer the nice display name from the TreeNode tag ===
                // (This now contains "hash.adx" for hashed ADX files thanks to OpenSingleFileAsAdx)
                ...
                {
                    suggestedName = tag.FileName;   // ← This is what gives us the .adx extension
                }
                ...
                {
                    // Force proper .adx extension and filter for ADX files
                ...
                if (exportFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        // For ADX files: temporarily disable metadata so we export raw bytes
                        if (isAdxFile)
                        {
                            ...
                            exportMetaData = originalExportMetaData; // restore original value
                        }
                        else
                        {
                            extractFile(currentRight, exportFileDialog.FileName);
